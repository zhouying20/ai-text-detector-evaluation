...
class HuggingFaceModel(ABC):
    def __init__(self, seq_max_len, batch_size, use_deepspeed=False, device="cuda") -> None:
        super().__init__()
        self.model = None
        self.tokenizer = None
    ...
    def eval_ppl(
        ...
        if add_start_token:
            ...
            # if not torch.all(torch.ge(attn_masks.sum(1), 2)):
            #     print("When add_start_token=False, each input text must be at least two tokens long. Run with add_start_token=True if inputting strings of only one token, and remove all empty input strings.")
            #     return [None]
            assert torch.all(
...
class LlamaPPLModel(HuggingFaceModel):
    def prepare_model(self,):
        self.model = AutoModelForCausalLM.from_pretrained(
...
def main():
    ...
    samples = [
        "lorem ipsum", "Happy Birthday!", "Bienvenue efgexb zhiu searcl"
    ]
