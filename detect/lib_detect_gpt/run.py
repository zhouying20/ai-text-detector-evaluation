...
def replace_masks(texts, mask_model, mask_tokenizer, device="cuda"):
...
def fallback_extracted_fills(origin_texts, perturbed_texts, masked_texts, extracted_fills):
    all_tokens = [x.split(' ') for x in masked_texts]
    n_expected = count_masks(masked_texts)

    res_texts = list()
    for idx, ptext in enumerate(perturbed_texts):
        if ptext == "":
            try:
                tokens = all_tokens[idx]
                n = n_expected[idx]
                fills = extracted_fills[idx]
                if len(fills) > 0:
                    print(f'WARNING: cur batch {idx}, only {len(fills)} fills. Trying to apply them, delete other words.')
                    for fill_idx in range(n):
                        mask_idx = tokens.index(f"<extra_id_{fill_idx}>")
                        if fill_idx < len(fills):
                            tokens[mask_idx] = fills[fill_idx]
                        else:
                            tokens[mask_idx] = "" # delete the word
                    res_texts.append(" ".join(tokens))
                else:
                    print(f'WARNING: cur batch {idx}, no fills generated. Fallbacking to original text.')
                    res_texts.append(origin_texts[idx])
            except Exception as e:
                print(f"ERROR: cur batch {idx}, skip it -> {e}.")
                continue
        else:
            res_texts.append(ptext)

    return res_texts


def perturb_texts_(texts, span_length, pct, mask_model, mask_tokenizer, device="cuda"):
    masked_texts = [tokenize_and_mask(x, span_length, pct) for x in texts]
    raw_fills = replace_masks(masked_texts, mask_model, mask_tokenizer, device=device)
    extracted_fills = extract_fills(raw_fills)
    perturbed_texts = apply_extracted_fills(masked_texts, extracted_fills)

    # Handle the fact that sometimes the model doesn't generate the right number of fills and we have to try again
    attempts = 1
    while '' in perturbed_texts and attempts <= 20: # TODO limit attempts at most 20
    ...
    if "" in perturbed_texts:
        perturbed_texts = fallback_extracted_fills(texts, perturbed_texts, masked_texts, extracted_fills)

    return perturbed_texts


def perturb_texts(texts, span_length, pct, mask_model, mask_tokenizer, device="cuda"):
    outputs = []
    for i in range(0, len(texts), 20):
        outputs.extend(perturb_texts_(texts[i:i + 20], span_length, pct, mask_model, mask_tokenizer, device=device))
    return outputs
...
def get_lls(texts, base_model, base_tokenizer, device="cuda"):
    if isinstance(base_model, str) and base_model.startswith("text-davinci"):
        get_ll_fn = functools.partial(get_ll, base_model=base_model, base_tokenizer=base_tokenizer, device=device)
        pool = ThreadPool(30)
        return pool.map(get_ll_fn, texts)
    else:
        return [get_ll(text, base_model, base_tokenizer, device=device) for text in texts]
...
# TODO
def get_perturbation_results(sequence, mask_model, mask_tokenizer, base_model, base_tokenizer, device="cuda", span_length=2, n_perturbations=100):
    perturb_fn = functools.partial(perturb_texts, span_length=span_length, pct=0.3, mask_model=mask_model, mask_tokenizer=mask_tokenizer, device=device)
    p_sampled_text = perturb_fn([sequence for _ in range(n_perturbations)])
    p_sequence_ll = get_lls(p_sampled_text, base_model, base_tokenizer, device=device)

    return {
        "original_ll": get_ll(sequence, base_model, base_tokenizer, device=device),
